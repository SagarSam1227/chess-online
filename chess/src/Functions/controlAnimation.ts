...
export function controlBishopAnimation(des:number,prev:number){
    ...
    if((prev-des)%9==0 && prev>des){
        ...
        const count = findingCount((prev-des),9)
        animationValue.from=`from {left: 0px; top: 0px; }`
        animationValue.to=`to {left: -${count*75}px; top: -${count*75}px; }`
    }
        ...
        const count = findingCount((des-prev),9)
        animationValue.from=`from {left: 0px; top: 0px; }`
        animationValue.to=`to {left: ${count*75}px; top: ${count*75}px; }`
    }
        ...
        const count = findingCount((prev-des),7)
        animationValue.from=`from {right: 0px; top: 0px; }`
        animationValue.to=`to {right: -${count*75}px; top: -${count*75}px; }`
    }
    
    
    else{
        animationName = 'center-to-down-left-corner'
        const count = findingCount((des-prev),7)
        animationValue.from=`from {right: 0px; top: 0px; }`
        animationValue.to=`to {right: ${count*75}px; top: ${count*75}px; }`
    }
